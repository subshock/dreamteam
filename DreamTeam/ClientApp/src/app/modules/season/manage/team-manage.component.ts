...
export class TeamManageComponent implements OnInit {
  ...
  constructor(private state: PublicSeasonStateService, private userApi: UserApiService, private route: ActivatedRoute,
    private router: Router) { }

  ngOnInit(): void {
    const playerObs = this.state.season$.pipe(
    ...
    this.model$ = combineLatest([this.state.season$, playerObs, teamObs]).pipe(
      map(([s, p, t]) => ({
  ...
  addPlayerToTeam(model: IModel, player: any) {
    ...
    if (this.getCurrentTeam(model).length >= model.season.maxPlayers ||
  ...
  isTeamValid(currentTeam: any[], model: IModel) {
    return currentTeam.length === model.season.maxPlayers
