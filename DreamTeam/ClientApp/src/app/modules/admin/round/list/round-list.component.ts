...
import { IRoundSummary } from '../../admin.types';
...
export class RoundListComponent implements OnInit, OnDestroy {
  ...
  rounds$: Observable<IRoundSummary[]>;

  private subscriptions: Subscription;

  deleteObj: IRoundSummary;
  modalRef?: BsModalRef;
  ...
  updateRound(round: IRoundSummary) {
    this.subscriptions.add(this.state.season$.pipe(take(1)).subscribe(s => {
  ...
  deleteRoundStart(round: IRoundSummary, template: TemplateRef<any>) {
